...
const approve_payout_dto_1 = require("../dto/approve-payout.dto");
...
let PayoutService = PayoutService_1 = class PayoutService {
    constructor(transactionRepository, escrowAccountRepository, paymentMethodRepository, payoutRequestRepository, disbursementRepository, dataSource, paymentProcessorService, notificationService, loanService) {
    ...
    async getUserPaymentMethods(userId, includeInactive = false) {
        const where = { userId };
        if (!includeInactive) {
            where.status = payment_method_enum_1.PaymentMethodStatus.ACTIVE;
        }
    ...
    async createPayoutRequest(createPayoutRequestDto) {
        ...
        Object.assign(payoutRequest, {
            ...
            priority: payout_enum_1.PayoutPriority.MEDIUM,
            currency: 'USD',
        });
    ...
    async approvePayout(id, approvedBy) {
        const approvePayoutDto = new approve_payout_dto_1.ApprovePayoutDto();
    ...
    async rejectPayoutRequestWithDto(payoutRequestId, rejectPayoutDto) {
    ...
    async processPayoutRequest(payoutRequestId) {
        ...
        try {
            ...
            if (payoutRequestToProcess.payoutMethod === payout_enum_1.PayoutMethod.BANK_TRANSFER ||
                payoutRequestToProcess.payoutMethod === payout_enum_1.PayoutMethod.WIRE_TRANSFER) {
                try {
    ...
    async processDisbursement(disbursementId, amount) {
        ...
        try {
            ...
            if (disbursementToProcess.escrowAccount) {
                transaction = await this.withdrawFromEscrow(disbursementToProcess.escrowAccount.id, disbursementAmount, `Disbursement ${disbursementToProcess.disbursementNumber} for loan ${disbursementToProcess.loan?.loanNumber || disbursementToProcess.loanId}`);
    ...
    async createDisbursementSchedule(disbursementId, installments) {
        const disbursement = await this.disbursementRepository.findOne({
            where: { id: disbursementId },
        });
        if (!disbursement) {
